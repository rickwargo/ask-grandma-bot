...
var AlexaUtterances = require('../alexa-utterances');   // TODO: Remove ../ when using node module
        ...
        for (intentName in self.intents) {
            ...
            if (intent.schema && intent.schema.slot_types) {
                intent.schema.slot_types.forEach(function (slot_type) {
                    var slotName = slot_type.name;
                    slotName = intent.schema.slots[slotName];
                    out += slotName + "\n";
                    out += slotName.replace(/./g, '-') + "\n";
